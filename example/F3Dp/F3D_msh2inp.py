...
def msh2inp(fin,dt_dyn):
    ...
    t_dyn=0.01; t_lim=1.; dsp=1; dsp_hyb=0; dsp_str=1; rsf=0 #; dt_dyn=0.00008
    bod_frc=0; hyb=1; nviz_dyn=120; nviz_wave=10; nviz_slip=500; init=1 #nviz_slip=2000 for ~100 m tet model
    ...
    flux = -1E5 #-7.5E4, -5E4
    if poro:
        fnodes = [[-.1, 0, -.35, 0, 0, 0, flux, 2*dt, 20*dt],
                  [ .1, 0, -.35, 0, 0, 0,    0, 2*dt, 15*dt]]
        ...
        for node_pos, node_neg, i in zip(ft_pos_nodes, ft_neg_nodes,\
            ...
            if node_pos != node_neg:
                ...
                if y < -.5 or y > .5:
                    ...
                    frc[j] = 1
                # Barrier patch
                d2cnt=np.sqrt((y-.0)**2+(z+.35)**2)
                if d2cnt>r: 
                    # st_init[j,2]=amp*np.cos(d2cnt/R*np.pi*.5)
                    st_init[j,2]=-min(amp,amp*(d2cnt-r)/(R-r))
    ...
    print 'critical L > %0.6f' %(10*dt_dyn*max(vp)*1E-3)+' km'

if __name__=='__main__':
    fin=sys.argv[1]
    dt_dyn=np.float(sys.argv[2])
    _=msh2inp(fin,dt_dyn)
