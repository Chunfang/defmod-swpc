...
dat_F3D  = load_dict_from_hdf5('x100/'+name_sol+'_fe.h5')
dat_seis = dat_F3D['obs_dyn']['step 1']
ocoord   = dat_F3D['crd_obs'] 
dt_dyn   = dat_F3D['dt_dyn']

# Read F3DX14 data
dat_F3DX=load_dict_from_hdf5('x200/'+name_sol+'_fe.h5')
dat_seis2=dat_F3DX['obs_dyn']['step 1']

nobs=11; dt=-.2; sign_df=[-1,1,-1]
t_plt=16.
#dat_DF=[dat_seis.item()[i] for i in range(nobs)]
dat_DF=[np.matrix.transpose(dat_seis[i,:,:]) for i in range(nobs)]
ang_dip=.5*np.pi
tol=1E-3
...
for i in range(nobs):
    ...
    for j in range(3):
        ...
        plt.plot(np.float64(range(len(dat)))*dt_dyn+dt,sign_df[j]*dat[:,j],label='x100',color='b',linewidth=1.)
        plt.plot(np.float64(range(dat2.shape[1]))*dt_dyn+dt,sign_df[j]*dat2[j,:],label='x200',color='g',linewidth=1.)
        if j==0:    
            plt.xlabel('t [s]')
            plt.ylabel('v [m/s]')
...
if rup==1 or dsp==1: # Plot rupture front
    ...
    fcoord   = dat_F3D['crd_flt'][:,:3] 
    dt_slip  = dat_F3D['dt_slip']
    dat_slip = dat_F3D['slip_dyn']['step 1']

    dat_slipX = dat_F3DX['slip_dyn']['step 1']
    fcoordX = dat_F3DX['crd_flt'][:,:3]

    id2=abs(fcoord[:,0])<tol; id1=abs(fcoord[:,0])>=tol
    ...
    for i in range(min(dat1.shape[1],datX1.shape[1])):
        t = i*dt_slip+dt
        #if dsp==1:
        #    slip = dat1[i,:]
        #else:
    ...
    for i in range(min(dat2.shape[1],datX2.shape[1])):
        t = i*dt_slip+dt
        #if dsp==1:
        #    slip = dat2[i,:]
        #else:
