...
ap.add_argument('-dt') # QS step in hr 
ap.add_argument('-pp') # production period in day (half on half off) 
ap.add_argument('-dsp')# absolute displacement output 0/1.

fin = ap.parse_args().m
dt_hr = int(ap.parse_args().dt)
df = int(ap.parse_args().pp)
dsp_hyb = int(ap.parse_args().dsp)
...
t_dyn = 0.025; dt_dyn=1.6E-5; t_lim = 5.; dsp=1; dsp_str=1; rsf=1; v_bg=1E-12
...
# Varying source (half df on half off) 
fnodes=np.empty([0,7],dtype=np.float)
for i in np.array(range(ndy))+(df==2):
...
if dsp_hyb==1:
    ...
    ext='.inp'
fout = fin.rsplit('.')[0]+'_dt%d'%(dt_hr) +'_pp%d'%(df) + ext 
print 'Write to ' + fout + '...'
